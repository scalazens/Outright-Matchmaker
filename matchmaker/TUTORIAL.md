...
jar and tell java to run it directly:
    ...
    $ sbt package-dist
    $ java -Dstage=development -jar ./dist/matchmaker/matchmaker-1.0.0-SNAPSHOT.jar
...
is named "matchmaker", there should be a `matchmaker.log` with contents like this:
...
The IDL for your service is in `src/main/thrift/matchmaker.thrift`.  The
...
your server running, as above, bring up a different shell and:
    ...
    $ cd matchmaker
    $ bundle install
    $ ./dist/matchmaker/scripts/console
    >> $client
...
requests. You can view the stats via that port:
    ...
    counters:
      Matchmaker/connects: 1
      Matchmaker/requests: 2
...
In `src/main/scala`, take a look at `MatchmakerServiceImpl.scala`. (This may
...
At the top of MatchmakerServiceImpl.scala, add:

    import com.twitter.ostrich.stats.Stats

Then inside get():

    Stats.incr("matchmaker.gets")

and inside put():

    Stats.incr("matchmaker.puts")
...
src/test/scala/com/twitter/matchmaker/MatchmakerServiceSpec.scala.  Check
...
command line.
    ...
    [info] Building project matchmaker 1.0.0-SNAPSHOT against Scala 2.8.1
    [info]    using MatchmakerProject with sbt 0.7.4 and Scala 2.7.7
